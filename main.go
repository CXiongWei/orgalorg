...
func getSSHPortEnv() int {
	...
	if err != nil {
		hierr.Errorf(err, "Error converting SSH_PORT to integer: %v", port)
	}
	return port
}
...
func createRunnerFactory(
	...
	if askPassword {
		password := os.Getenv("ORGALORG_PASSWORD")
		if password == "" {
			return nil, fmt.Errorf("can't read password%s", password)
		}
